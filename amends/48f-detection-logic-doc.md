# 48f — DETECTION_LOGIC.md (Step 6)

Step 5 (`amends/48e-multisensor-backend-wiring.md` +
`amends/48e-2-recreate-adapt-to-real-schema.md`) was confirmed working, so
this proceeds as instructed.

## What was written

`DETECTION_LOGIC.md`, at the repo root (no existing root-level doc
convention to match — no other `*.md` files exist there — so this is the
first one, named for what it documents rather than nested under `amends/`,
since it's a durable reference doc, not a session log).

Explains the **multi-sensor agreement principle** in plain language, for a
non-engineer reader (product/clinical audience, not just developers):

1. **The problem**: heart rate alone can't distinguish exercise from panic
   — both push it into the same range. Grounded in a real, checkable fact
   from this project rather than a generic claim: 48b's own validation run
   flagged `false_alarm` and `real_panic`'s mean heart rates as within 3bpm
   of each other.
2. **The principle**: the system looks for several sensors agreeing on the
   same story, not one loud signal. A comparison table walks through what
   each of the 6 sensors (heart rate, motion, EDA/GSR, skin temperature,
   PRV, SpO2) reports under exercise vs. real panic, in plain terms (e.g.
   "blood flows toward the skin to cool down" instead of "vasodilation").
3. **How it's actually decided**: a trained model, not hand-written rules
   — with the real, honest numbers from 48d (97.3% accuracy, zero
   false_alarm/real_panic confusion in either direction on 45 held-out
   patients), not a made-up or rounded-up figure.
4. **Graceful degradation**: what happens on a 3-sensor wearable missing
   EDA/skin-temp/PRV — imputed defaults from 48e, not a crash, and not an
   all-or-nothing gate on the decision.
5. A one-line takeaway closing the piece.
...
The file exists and reads clearly.

**Step 6 CONFIRMED WORKING.**

## Scope guardrail

Only `DETECTION_LOGIC.md` (new file, repo root) was created. No code, data,
or model files touched.
